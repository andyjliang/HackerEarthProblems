...
n = int(raw_input())
adj = zeros(shape=(n,n),dtype=int)

# Create primary link adj list with set of cities for each city
for i in range(n):
...
for x1, row1 in enumerate(adj):
	row_comb = list(combinations([i for i, x in enumerate(row1) if x == 1], 2))
	row_comb = set([tuple(sorted(each_comb)) for each_comb in row_comb])
	# print row_comb, lateral_paths
	print row_comb.intersection(lateral_paths)
	gardens += len(row_comb.intersection(lateral_paths))
	lateral_paths = lateral_paths.union(row_comb)
