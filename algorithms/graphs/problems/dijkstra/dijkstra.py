...
class Network(object):
    ...
    def verify_path(self, path, source, destination):
        ...
        """
        if source != path[0]:
            raise ValueError('First node on a path is different form the source.')
        if destination != path[-1]:
            raise ValueError('Last node on a path is different form the destination.')
        for i in range(len(path) - 1):
            if (path[i], path[i+1]) not in self.edge_set and \
                (path[i+1], path[i]) not in self.edge_set:
                raise ValueError('Adjacent nodes in path have no edge between them')
        return True
